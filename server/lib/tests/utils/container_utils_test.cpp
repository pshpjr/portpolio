...
TEST(ContainerUtilsTest, EraseIfRemovesMatchingElements)
{
    ...
    const auto removed = erase_if(values,
                                  [](const int value)
                                  {
                                      return value % 2 == 0;
                                  });
...
TEST(ContainerUtilsTest, TransformToVecCollectsProjectedValues)
{
    ...
    const auto transformed = transform_to_vec(values,
                                              [](const int value)
                                              {
                                                  return value * value;
                                              });
...
TEST(ContainerUtilsTest, KeysAndValuesExposeMapViews)
{
    ...
    const auto keyList = transform_to_vec(keys(table),
                                          [](const auto& key)
                                          {
                                              return key;
                                          });
    const auto valueList = transform_to_vec(values(table),
                                            [](const auto& value)
                                            {
                                                return value;
                                            });
