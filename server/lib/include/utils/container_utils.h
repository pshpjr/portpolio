...
[[nodiscard]] constexpr auto find_if_or(Range&& range,
                                        Predicate predicate,
    ...
    return static_cast<std::ranges::range_value_t<Range>>(
        std::forward<DefaultValue>(defaultValue));
}
...
[[nodiscard]] constexpr auto transform_to_vec(Range&& range, Function function) {
    using ResultType = std::remove_cvref_t<
        std::invoke_result_t<Function&, std::ranges::range_reference_t<Range>>>;
