...
public:
    ...
    constexpr explicit operator UInt64() const { return entityId_; }

    [[nodiscard]] auto GetEntityId() const -> EntityId { return EntityId(entityId_); }
    void SetEntityId(const EntityId& entityId) { entityId_ = static_cast<UInt64>(entityId);}
    //
...
template <std::integral I> constexpr bool operator==(const EntityId& lhs, I rhs) { return static_cast<std::uint64_t>(lhs) == rhs; }
template <std::integral I> constexpr bool operator==(I lhs, const EntityId& rhs) { return lhs == static_cast<std::uint64_t>(rhs); }
