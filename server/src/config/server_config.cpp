...
std::string requireEnv(const char* key)
{
    ...
    spdlog::error(
        "[config] Set it in your environment or copy .env.example to .env and fill in the values.");
    std::exit(1);
}
...
ServerConfig ServerConfig::Load(const std::filesystem::path& jsonPath)
{
    ...
    if (std::ifstream f{jsonPath}; f.is_open())
    {
        ...
        if (j.contains("server_name"))
            cfg.serverName = ServerName{j["server_name"].get<std::string>()};
        if (j.contains("shard_id"))
            cfg.shardId = ShardId{j["shard_id"].get<UInt32>()};
        if (j.contains("port"))
            cfg.port = Port{j["port"].get<UInt16>()};
        if (j.contains("db_host"))
            cfg.dbHost = DbHost{j["db_host"].get<std::string>()};
        if (j.contains("db_user"))
            cfg.dbUser = DbUser{j["db_user"].get<std::string>()};
        if (j.contains("db_password"))
            cfg.dbPassword = DbPassword{j["db_password"].get<std::string>()};
        if (j.contains("redis_url"))
            cfg.redisUrl = RedisUrl{j["redis_url"].get<std::string>()};
        if (j.contains("service_endpoint"))
            cfg.serviceEndpoint = ServiceEndpoint{j["service_endpoint"].get<std::string>()};
        if (j.contains("feature_flags"))
            cfg.featureFlags = j["feature_flags"].get<std::string>();
        if (j.contains("log_level"))
            cfg.logLevel = LogLevel{j["log_level"].get<std::string>()};
    }
    ...
    if (auto v = env("SERVER_NAME"))
    {
        ...
        if (next != cfg.serverName)
            logOverride("SERVER_NAME", cfg.serverName, next);
        cfg.serverName = next;
    }
    if (auto v = env("SHARD_ID"))
    {
        ...
        if (next != cfg.shardId)
            logOverride("SHARD_ID", cfg.shardId, next);
        cfg.shardId = next;
    }
    if (auto v = env("SERVICE_ENDPOINT"))
    {
        auto next = ServiceEndpoint{*v};
        if (next != cfg.serviceEndpoint)
            logOverride("SERVICE_ENDPOINT", cfg.serviceEndpoint, next);
        cfg.serviceEndpoint = next;
    }
    if (auto v = env("FEATURE_FLAGS"))
    {
        if (*v != cfg.featureFlags)
            logOverride("FEATURE_FLAGS", cfg.featureFlags, *v);
        cfg.featureFlags = *v;
    }
    if (auto v = env("LOG_LEVEL"))
    {
        ...
        if (next != cfg.logLevel)
            logOverride("LOG_LEVEL", cfg.logLevel, next);
        cfg.logLevel = next;
    }
