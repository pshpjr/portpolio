...
- 항목이 **20개 이상** 쌓이거나 파일이 **600줄**을 초과하면 오래된 항목을 `harness-feedback-log-archive.md`로 이동한다.
- 아카이브 기준: 가장 오래된 항목부터, 이미 개선 조치가 완료된 항목 우선 이동.
- 아카이브 파일은 날짜 역순(최신이 위)으로 정렬한다.
...
## 2026-04-05 - harness-token-optimization-session
- kind: process-gap
- area: workflow
- summary: 세션 시작 시 로드되는 agent-workflow.md(1,666t)가 단일 문서로 너무 크고, feedback-log는 항목이 쌓이면서 계속 증가하는 구조였다.
- impact: 매 세션마다 수백~천 단위 토큰이 낭비될 수 있고, 문서 로드 부하를 추정할 수 없었다.
- suggested-follow-up: agent-workflow.md를 quick-ref + detail로 분리(완료), feedback-log 아카이브 규칙 추가(완료), doc_token_audit.py 작성은 improvement-queue에 등록.
