...
- 개인 드롭, 장비 수리, 제한적 재련, 계정 귀속(창고 이동 제약), 정화/정찰 원정으로 나뉘는 최소 경제 루프 (유저 간 거래는 v2 이후)
- 단일 스레드 인던 루프와 성능 리포트로 설명 가능한 서버 기술 데모

### v1 데모 계약

- 보스는 `1페이즈 코어 패턴 + 무력화 딜 타임 → 2페이즈 전환 → 강화 패턴 + 협동 기믹`의 완전한 2페이즈 구조를 보여준다.
- 전투 문서에는 카메라 거리, 화면 구도, HUD, 보스 전조 가독성 같은 핵심 플레이 표현 요소를 포함한다.
- 무기 변경은 가능하지만 `마을 또는 입장 준비 구간`에서만 허용하고, 던전 입장 후에는 현재 무기와 스킬 구성이 잠긴다.
- 경제는 `수리 + 제한적 재련 + 계정 귀속(창고 제약)`을 기본으로 설명한다. 유저 거래는 v2 이후로 미룬다.
...
- 보스 2페이즈 세부 패턴 확장(소환 압박 강화, 복잡한 강제 타게팅)
