...
- 마을 1곳, 고정 던전 1종, 보스 1종으로 이어지는 완결된 플레이 루프
- 검방, 대검, 지팡이 3개 무기 타입과 각 무기별 아이덴티티
- `패링/카운터`, `무력화/다운`, `장착 무기 기반 스마트 드롭`이 보이는 전투/보상 구조
- 개인 드롭, 장비 수리, 제한된 재련, 거래 횟수 기반 귀속 전환까지 포함한 최소 경제 루프
...
- 드롭은 장착 무기에 유리한 soft bias를 주되, 오프빌드와 공용 재화 드롭 여지는 남긴다.
